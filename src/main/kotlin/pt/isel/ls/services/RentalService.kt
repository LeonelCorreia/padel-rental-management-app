...
class RentalService(
    ...
    fun numberOfRentals(
        ...
        runCatching {
            trxManager.run {
                PaginationInfo(rentalRepo.numRentalsOfCourt(crid, date))
            }
        }
    ...
    fun numberOfUserRentals(uid: UInt): Result<PaginationInfo> =
        runCatching {
            trxManager.run {
                PaginationInfo(rentalRepo.numRentalsOfUser(uid))
            }
        }
    ...
    fun getUsersOfRentalsOnCourt(
        crid: UInt,
    ): Result<Map<User, UInt>> =
        runCatching {
            trxManager.run {
                rentalRepo.getNumOfRentalsOfUsersOnCourt(crid)
            }
        }
}
